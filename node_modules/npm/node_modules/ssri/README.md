...
// your API.js file so you can use a single integrity string and serve
...
```javascript
ssri.fromStream(fs.createReadStream('API.js'), {
  algorithms: ['sha1', 'sha512']
}).then(integrity => {
  return ssri.checkStream(fs.createReadStream('API.js'), integrity)
}) // succeeds
```
...
```javascript
const data = fs.readFileSync('API.js')
...
```javascript
const integrity = ssri.fromData(fs.readFileSync('API.js'))

ssri.checkStream(
  fs.createReadStream('API.js'),
  integrity
)
// ->
// Promise<{
